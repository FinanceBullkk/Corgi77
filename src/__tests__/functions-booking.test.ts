...
describe('Cloud Functions booking handlers', () => {
  ...
  it('rejects booking requests from non-company email domains', async () => {
    await expect(fns.bookRegistration({
      ...signed('user@gmail.com'),
      data: basePayload(),
    })).rejects.toMatchObject({
      code: 'permission-denied',
      message: expect.stringContaining('@cyberlogitec.com'),
    });
  });

  it('rejects cancellation requests from non-company email domains', async () => {
    await expect(fns.cancelRegistration(signed('user@gmail.com'))).rejects.toMatchObject({
      code: 'permission-denied',
      message: expect.stringContaining('@cyberlogitec.com'),
    });
  });
  ...
  it('rejects empCode already claimed by another email', async () => {
    seedOpenConfig(db);
    seedSlots(db);
    db.store.set('empCodeClaims/262010', { email: 'other@cyberlogitec.com' });
  ...
  it('rejects BU outside the default list when config buList is empty', async () => {
    ...
    await expect(fns.bookRegistration({
      ...signed('outsider@cyberlogitec.com'),
  ...
  it('creates a new booking, claim, slot decrements and audit log', async () => {
    ...
    expect(db.store.get('empCodeClaims/262010')).toEqual({ email: 'user@cyberlogitec.com' });
  ...
  it('updates an existing booking, restores old slots and deletes old empCode claim', async () => {
    ...
    db.store.set('registrations/user@cyberlogitec.com', {
      email: 'user@cyberlogitec.com',
      empCode: '262009',
      fullName: 'Old Name',
      bu: 'BSG',
    ...
    db.store.set('empCodeClaims/262009', { email: 'user@cyberlogitec.com' });
  ...
  it('escapes confirmation email HTML and sends to authenticated email only', async () => {
    ...
    const result = await fns.bookRegistration({
      ...signed('attacker@cyberlogitec.com'),
    ...
    expect(mail?.to).toBe('attacker@cyberlogitec.com');
    expect(mail?.message.html).toContain('&lt;script&gt;alert(1)&lt;/script&gt;');
    expect(mail?.message.html).not.toContain('<script>alert(1)</script>');
    expect(mail?.message.html).toContain('Google Calendar');
    expect(mail?.message).not.toHaveProperty('attachments');
  });
  ...
  it('rate limits rapid cancellation attempts', async () => {
    ...
    db.store.set('functionRateLimits/cancelRegistration_user@cyberlogitec_com', {
  ...
  it('repairs empCode claims and reports duplicates/conflicts', async () => {
    ...
    db.store.set('registrations/a@cyberlogitec.com', { empCode: '262001' });
    db.store.set('registrations/b@cyberlogitec.com', { empCode: '262002' });
    db.store.set('registrations/c@cyberlogitec.com', { empCode: '262002' });
    db.store.set('registrations/d@cyberlogitec.com', { empCode: '262003' });
    db.store.set('empCodeClaims/262003', { email: 'someone@cyberlogitec.com' });

    const result = await fns.repairEmpCodeClaimsNow(signed('admin@cyberlogitec.com'));

    expect(result.created).toBe(1);
    expect(result.skippedDuplicates).toEqual([{ empCode: '262002', emails: ['b@cyberlogitec.com', 'c@cyberlogitec.com'] }]);
    expect(result.conflicts).toEqual([
      { empCode: '262003', claimEmail: 'someone@cyberlogitec.com', registrationEmail: 'd@cyberlogitec.com' },
    ]);
    expect(db.store.get('empCodeClaims/262001')).toEqual({ email: 'a@cyberlogitec.com' });
  ...
  it('cleans redundant email fields from registration documents', async () => {
    seedOpenConfig(db, { adminEmails: ['admin@cyberlogitec.com'] });
    db.store.set('registrations/user@cyberlogitec.com', { email: 'user@cyberlogitec.com', empCode: '262010' });
    db.store.set('registrations/clean@cyberlogitec.com', { empCode: '262011' });

    const result = await fns.cleanupRegistrationEmailFieldsNow(signed('admin@cyberlogitec.com'));

    expect(result).toEqual({ scanned: 2, cleaned: 1 });
    expect(db.store.get('registrations/user@cyberlogitec.com')).toEqual({ empCode: '262010' });
    expect(db.store.get('registrations/clean@cyberlogitec.com')).toEqual({ empCode: '262011' });
