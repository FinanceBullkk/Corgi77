...
2. **`prototype/01-hifi/Booking Flow Hi-fi.html`** — clickable prototype (user flow)
...
2b. **`prototype/01-hifi/Admin Panel.html`** — clickable prototype (admin)
   - Sidebar nav · 6 tab · data grid · CRUD ca thi · audit người đọc
   - Mở trực tiếp, không cần build
...
5. **`ADMIN_REDESIGN.md`** — ⭐ redesign toàn bộ **Admin Panel** (sidebar + data grid + audit người đọc + CRUD ca thi). Prototype: `prototype/01-hifi/Admin Panel.html`. Đọc trước khi build admin.
...
| `admin-shell.css` · `admin.css` | Admin: shell sidebar + data grid + settings controls |
| `admin-data.js` · `admin-app.js` | Admin: mock domain + render/tương tác (xem `ADMIN_REDESIGN.md`) |
