...
> Files đụng tới: `step2-calendar.jsx` (phần render lịch) + `calendar.css` (toàn bộ grid & block). Logic chọn ca / auto-switch tab **không đổi**. **Cập nhật mới:** slot trùng giờ với ca đã chọn ở tab kia giờ **ẩn hẳn** thay vì vẽ block "Trùng giờ" mờ (xem §4 bảng trạng thái + §6 QA).
...
| Trùng giờ với ghost | **ẩn hẳn** (`if (status==='conflict') return null`) | KHÔNG render. Ghost block đã thể hiện khung giờ bị khoá → không vẽ thêm block "Trùng giờ" chồng lên (tránh hiện cả "đã chọn" lẫn "trùng giờ"). Class `.ev.conflict` giữ lại cho trường hợp edge nhưng không dùng tới. |
...
- [ ] Chọn 1 ca 3 Skills → xem tab Speaking → các ca Speaking **trùng giờ** với ghost **biến mất hẳn** (không còn block "Trùng giờ" mờ chồng lên ghost). Chỉ còn ghost block thể hiện khung giờ đã khoá.
- [ ] Ca **không** trùng giờ vẫn hiện bình thường.
